...
Then(/^sets the language to english$/, () => {
  return generics.languageToggle(cssLib.topNav.languages.english());
});
...
Then(
  /^the user enters the email: (.*?), skype: (.*?), company phone: (.*?), personal phone: (.*?), car number: (.*?)$/,
  (email, skype, workPhone, personalPhone, carNumber) => {
...
Then(
  /^profile is updated as it follows, email: (.*?), skype: (.*?), company phone: (.*?), personal phone: (.*?), car number: (.*?)$/,
  async (email, skype, workPhone, personalPhone, carNumber) => {
    const list = [];
...
Then(
  /^profile should not be updated as it follows, email: (.*?), skype: (.*?), company phone: (.*?), personal phone: (.*?), car number: (.*?)$/,
  async (email, skype, workPhone, personalPhone, carNumber) => {
...
Then(
  /^the user enters the first name: (.*?), last name: (.*?), birthdate: (.*?)$/,
...
When(/^genders are visible$/, () => {
  return client.assert
    .visible(cssLib.userProfile.childrenSection.genderMale())
...
Then(
  /^the childe with the following data, firstname: (.*?), lastname: (.*?), birthdate: (.*?), gender: (.*?) is displayed$/,
...
Then(
  /^the childe with the following data, firstname: (.*?), lastname: (.*?), birthdate: (.*?), gender: (.*?) should not be displayed$/,
  async (firstname, lastname, birthdate) => {
    const age = userProfile.ageCalculator(birthdate);
    const ageString = age.toString();
